...
class Test:
    ...
    def __init__(self):
        ...
        self.personGroupId = storeData().createPersongroup()
    ...
    def faceidentify(self, img_bgr,faceID, left, top, bottom, right):
        faceIdsList = [faceID]
        faceIdsList.append(faceID)
        # Request Body
        body = dict()
        body["personGroupId"] = self.personGroupId
        ...
        FaceApiGetPerson = self.endpoint+self.personGroupId+'/persons/'+personId
        ...
        try:
            ...
            mycol=mydb["person"]
            testquery={"personId":str(responseJson["personId"])}
            print(testquery)
            find=mycol.find_one(testquery)
            print(find)
            name=find['name']
            ...
            print("This Is "+name)
            cv2.putText(img_bgr,access,(left,top-10),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
